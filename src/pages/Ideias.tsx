...
const IdeiasPage = () => {
  ...
  const handleCreate = async (e: React.FormEvent, status: string = 'Pendente') => {
    e.preventDefault();
    setSaving(true);
    const { error } = await supabase.from('ideias').insert({ assunto_tema: form.assunto_tema, formato: form.formato, observacoes: form.observacoes || null, plataforma: form.plataforma, data_uso: form.data_postagem || null, status });
  ...
  const handleKanbanMove = async (itemId: string | number, newStatus: string) => { await handleStatusChange(Number(itemId), newStatus); };

  const platformIcon = (p: string) => { const plat = PLATAFORMAS.find(pl => pl.value === p); if (!plat) return null; const Icon = plat.icon; return <Icon className="w-3.5 h-3.5" />; };

  const columns: KanbanColumn<Ideia>[] = STATUS_COLS.map(col => ({ id: col.id, title: col.title, color: col.color, items: ideias.filter(i => i.status === col.id) }));
  ...
  const renderKanbanCard = (ideia: Ideia) => (
    <div className="bg-card rounded-lg border border-border p-3 space-y-2 hover:shadow-sm transition-shadow">
      <div className="flex items-start gap-2">
        <span className="text-muted-foreground">{platformIcon(ideia.plataforma ?? 'linkedin')}</span>
        <p className="text-sm font-medium text-foreground flex-1">{ideia.assunto_tema}</p>
      </div>
      <div className="flex items-center gap-2 flex-wrap">
        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">{ideia.formato}</span>
          ...
          <div key={ideia.id} className="bg-card rounded-lg border border-border p-4 space-y-2.5">
            ...
            {ideia.status === 'Pendente' && (
              <div className="flex gap-2 pt-1">
                <Button size="sm" className="flex-1 gap-1" onClick={() => handleStatusChange(ideia.id, 'Em uso')}><Check className="w-3 h-3" />Usar</Button>
                <Button size="sm" variant="outline" className="flex-1 gap-1 text-destructive" onClick={() => handleStatusChange(ideia.id, 'Rejeitado')}><X className="w-3 h-3" />Rejeitar</Button>
              </div>
            )}
            {ideia.status === 'Em uso' && (
              <Button size="sm" variant="outline" className="w-full" onClick={() => handleStatusChange(ideia.id, 'Usado')}>Marcar como Usado</Button>
            )}
          </div>
        ))}
      </div>
              ...
              <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleCreate} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button type="button" variant="outline" className="flex-1" disabled={saving} onClick={(e) => handleCreate(e as any, 'Em uso')}>Salvar e Usar</Button>
                  </div>
                </form>
              </DialogContent>
            </Dialog>
