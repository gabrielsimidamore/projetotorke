...
import {
  ...
  Music, File, StickyNote, ChevronRight, Home, X, Megaphone,
} from 'lucide-react';
...
export default function TrafegoPago() {
  ...
  const load = async (pastaId: string | null) => {
    const [{ data: ps }, { data: arqs }, { data: nts }] = await Promise.all([
      supabase.from('pastas').select('*').eq('tipo', 'trafego_pago').is('parent_id', pastaId ?? null).order('nome'),
      supabase.from('arquivos').select('*').eq('bucket', BUCKET).is('pasta_id', pastaId ?? null).order('created_at', { ascending: false }),
      supabase.from('notas').select('*').eq('tipo', 'trafego_pago').is('pasta_id', pastaId ?? null).order('fixada', { ascending: false }),
    ]);
    setPastas(ps ?? []);
    setArquivos(arqs ?? []);
    setNotas(nts ?? []);
  ...
  const entrarPasta = (p: Pasta) => { setBreadcrumb(prev => [...prev, p]); setPastaAtual(p.id); };
  ...
  const criarPasta = async () => {
    ...
    await supabase.from('pastas').insert({ nome: nomePasta, cor: corPasta, tipo: 'trafego_pago', parent_id: pastaAtual });
    setNomePasta(''); setDlgPasta(false); load(pastaAtual);
  };

  const deletarPasta = async (id: string) => { await supabase.from('pastas').delete().eq('id', id); load(pastaAtual); };
  ...
  const uploadArquivo = async (file: File) => {
    ...
    setUploading(false); load(pastaAtual);
  };
  ...
  const deletarArquivo = async (arq: Arquivo) => {
    ...
    setDlgArquivo(null); load(pastaAtual);
  };
  ...
  const salvarNota = async () => {
    if (editNota) {
      ...
      await supabase.from('notas').insert({ titulo: tituloNota, conteudo: conteudoNota, tipo: 'trafego_pago', pasta_id: pastaAtual });
    }
    setDlgNota(false); setEditNota(null); setTituloNota(''); setConteudoNota(''); load(pastaAtual);
  };

  const deletarNota = async (id: string) => { await supabase.from('notas').delete().eq('id', id); load(pastaAtual); };
  const abrirNota = (n: Nota) => { setEditNota(n); setTituloNota(n.titulo); setConteudoNota(n.conteudo ?? ''); setDlgNota(true); };
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Criativos, estratégias, relatórios e notas</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {pastas.map(p => (
                <div key={p.id} className="group relative glass-card p-3 flex flex-col items-center gap-2 cursor-pointer hover:scale-105 transition-transform"
                  ...
                  <span className="text-xs font-medium text-foreground truncate w-full text-center">{p.nome}</span>
        ...
        {pastas.length === 0 && arquivos.length === 0 && notas.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-muted-foreground">Nenhum arquivo ainda. Crie uma pasta ou faça upload de criativos.</p>
          </div>
        )}
      </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <Label>Nome</Label>
              <Input value={nomePasta} onChange={e => setNomePasta(e.target.value)} placeholder="Ex: Meta Ads — Março" />
            </div>
            <div className="space-y-1">
              <Label>Cor</Label>
      ...
      <Dialog open={dlgNota} onOpenChange={v => { setDlgNota(v); if (!v) { setEditNota(null); setTituloNota(''); setConteudoNota(''); } }}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <Label>Título</Label>
              <Input value={tituloNota} onChange={e => setTituloNota(e.target.value)} placeholder="Ex: Estratégia Google Ads" />
            </div>
            <div className="space-y-1">
              <Label>Conteúdo</Label>
              <Textarea value={conteudoNota} onChange={e => setConteudoNota(e.target.value)}
                placeholder="Escreva sua nota, estratégia, briefing…" rows={8} />
            </div>
